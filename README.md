...
In figure 4 we examine the relation of one social media variable as regressor or independent variable and a mental health variable as dependent or outcome variable. It depect a positve but low relationship between the two variables similar to the low correlation coeficeint which is 0.356. Furthermore the coefficent of the model is 0.39 with an intercept of 1.95 and R-squre of 0.13. The R-square is low which indicate that only 13 % of the variation in dependent variable can be explained by independent variable. 

### Regression Analysis
In order to further analyze the relationship between what we call social media variables and menatal health varibles we run two types of regression one with simple linear regression and another with multiple linear regression. In the simple linear regression we calculated for each social media variable as independent variable and for each mental health variable as dependent variable. Doing so we found that variablity of most of the menatal health variables are not explained with variablity of social media variables, i.e. most of the R-squares of these regressions are bellow 0.2. Some R-square are as low as 0.007 or lesser. Only two mental health variables have R-squares above 0.2 with two social media variables. They are as following: 'how easily distracted one is' as dependent variable and 'getting distracted by social media when one is busy doing something' as independent variable has the highest R-square of simple regressions we run which is 0.37 that means that goodness of fit for this regression model is 37 % the coefficeint and intercept are 0.54 and 1.56 respectively. 

Other dependent variable is 'finding difficulty to concentrate on things'
with the same independent variable as before which has R-square above 0.2 (i.e. 0.29) with coefficeint and intercept of 0.54 and 1.47 respectively. The above two menationed dependent variables namely 'how easily distracted one is' and 'finding difficulty to concentrate on things' wile 'feeling restless if one is not having used social media in a while' being as independent variable have R-squre score above 0.2 (i.e. 0.27 and 0.22 respectively) with coefficeint of 0.48 and 0.5 as well as intercept of 2.1 and 1.94 respectively.

In order to look depeer and examine if two or more of the social media variables at the same time may explain the data better and R-squared is improved, we run multiple linear regression models. When doing so we found that even multi-linear regression does not help us much in improving our R-square for the rest of the dependent variable other than those two variables with R-squared higher than 0.2 in simple linear regression except variable 'fluctuation of interest in daily activities' which has improved R-square in multi-linear regression above 0.2. 

We run three types of multi-linear regressions first we run a model of each dependent variable with all four social media variables as independent variables. In second step by observing that the variable 'average time spent on social media' has P-values less than 0.05 and in some cases even greater than 0.1 with coefficeint of no greater than 0.13 make it statistically insignificant, we omit this variable and run our regression only on the three remaining social media variables. Again we observe that the variable 'finding oneself using social media without a specific purpose' also suffers from the same statistical insegnificant as previous one in most of the models, so we omit this variable as well and run another regression with two social media variables as independent variables.

In last regression model with two independent variables the result are as following: 
Dependent variable 'how easily distracted one is' with independent variables 'getting distracted by social media when one is busy doing something' and 'feeling restless if one is not having used social media in a while' have an adjusted R-square of 0.42 with coefficients of 0.41 and 0.26 for each independent variables respectively as well as intercept of 1.31 while P-value for all three of them are 0.000. Next dependent variable that has higher R-square above 0.2 is 'finding difficulty to concentrate on things' with same independent variables as before with adjusted R-square of 0.33 and with coefficients of 0.39 and 0.29 and intercept of 1.19 while P-value for all three of them are 0.000.

Another dependent variable that had R-square bellow 0.2 in simple regression that its R-square has improved as a result of multiple linear regression is 'fluctuation of interest in daily activities' while the independent variables are same as above two models. Adjusted R-square is 0.2 and coefficients are 0.22 and 0.18 and intercept is 1.23 whit P-value of the three of them being 0.000. 
...
Visula analysis and Regression analysis also confirms the relationship found in correlation analysis.
